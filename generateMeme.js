    ...
    [
      {
        ...
        explanation: "Generate meme with custom font styling - gold text with brown outline",
      },
    ],
  ],
  schema: z.object({
    ...
    fontSize: z
      .number()
      .optional()
      .describe("Font size in pixels. Auto-calculated based on image width if not provided"),
    fontColor: z
      .string()
      .optional()
      .describe("Font color (default: white). Hex color code like #FFFFFF or color name like 'white'"),
    strokeColor: z
      .string()
      .optional()
      .describe("Stroke/outline color (default: black). Hex color code like #000000 or color name like 'black'"),
    strokeWidth: z
      .number()
      .optional()
      .describe("Stroke width in pixels (default: auto-calculated). Makes text more readable"),
  }),
  handler: async (keypair, inputs) => {
    let actionMessage = `[TOOL] Generating meme`;

    try {
      const { prompt, imageUrl, topText, bottomText, fontSize, fontColor, strokeColor, strokeWidth } = inputs;
